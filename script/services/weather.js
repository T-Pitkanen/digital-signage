export function getWeather() {
	...
	function showWeatherData(data) {
		...
		const { maxtemp_c: tempMax, mintemp_c: tempMin } =
		...
		const [day2Data, day3Data] = data.forecast.forecastday
			.slice(1)
			.map(({ day: { maxtemp_c: max, mintemp_c: min } }) => ({ max, min }));
		...
		for (let i = 0; i < iconEls.length; i++) {
			const iconData = data.forecast.forecastday[i].day.condition.text;
			const iconName = icons[iconData] || 'unknown.png';
